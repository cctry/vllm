...
def push_kv_cache(request_id: str, blocks, kv_addr):
    block_ids = [block.block_number for block in blocks]
    task = asyncio.create_task(
        call_kv_method(engine, "push_kv", request_id, block_ids, kv_addr)
    )
    task.add_done_callback(lambda _: free_blocks(blocks))
    return task
